...
inputdir <- ".\\data_ISCOLE"
outputdir <- ".\\output"
outputfile <- paste("COL PA ISCOLE var_",as.Date(Sys.time()),".csv", sep="")
...
stime <- times("7:00:00")
etime <- times("14:00:00")
dd <- subset(data60, data60$hour >= hours(stime) & data60$hour<hours(etime) & data60$weekday == "domingo")
dd2 <- subset(data60, data60$hour >=hours(stime) & data60$hour<hours(etime) & data60$weekday == "lunes")
...
axis(2)
legend(x= c(etime-times("01:15:00")*etime/times("14:00:00"),etime-times("00:30:00")*etime/times("14:00:00")),y=c(4800,6000),legend=c("Sunday","Monday"),lty=1, lwd=3,
...
stime <- times("1:00:00")
etime <- times("23:00:00")
dd <- subset(data60, data60$hour >= hours(stime)-1 & data60$hour<hours(etime)+1 & data60$weekday == "domingo")
dd2 <- subset(data60, data60$hour >=hours(stime)-1 & data60$hour<hours(etime)+1 & data60$weekday == "lunes")

plot(dd$time,dd$smooth, type="l", lwd=3,col=rgb(166/250,34/255,30/255),
     xlab="Time", ylab="Counts per minute (cpm)",
     xlim = c(stime,etime),ylim=c(0,6000), axes = F, main="Sunday")
abline(h=c(100,573*4,1002*4), col = "gray", lty = "dotted",lwd = 3)
text(x=rep(stime-times("00:10:00"),3),y=c(573*4,1002*4,6150)-150,adj = c(0,1),
     labels=c("Light","Moderate","Vigorous"))
lines(dd$time,dd$smooth,lwd=3,col=rgb(166/250,34/255,30/255))
axis(side=1,at=seq(stime,etime,times("02:00:00")),
     labels= paste(seq(hours(stime),hours(etime),2),":00",sep=""))
axis(2)

box()

plot(dd2$time,dd2$smooth, type="l", lwd=3,col=rgb(49/250,76/255,117/255),
     xlab="Time", ylab="Counts per minute (cpm)",
     xlim = c(stime,etime),ylim=c(0,6000), axes = F, main="Monday")
abline(h=c(100,573*4,1002*4), col = "gray", lty = "dotted",lwd = 3)
text(x=rep(stime-times("00:10:00"),3),y=c(573*4,1002*4,6150)-150,adj = c(0,1),
     labels=c("Light","Moderate","Vigorous"))
lines(dd2$time,dd2$smooth,lwd=3,col=rgb(49/250,76/255,117/255))
...
days <- c("sábado","domingo","lunes","martes","miércoles","jueves","viernes")
days <- c("domingo","lunes", "martes")


par(mfrow=c(1,3))
for(day in days){
  dd <- subset(data60, data60$hour >=7 & data60$weekday == day)
  plot(dd$time,dd$axis1, type="l", lwd=2,col=rgb(166/250,34/255,30/255),
       xlab=day, ylab="counts per minute",
       xlim = c(times("7:00:00"),times("14:00:00")),ylim=c(0,10000))
}




unique(as.Date(data60$datetime))
cbind(unique(as.Date(data60$datetime)),weekdays(unique(as.Date(data60$datetime))))
